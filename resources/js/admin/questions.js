...
function buildQuestionCard(q) {
    ...
    <div id="question-card-${q.id}" class="border rounded-lg p-4 border-yellow-300 bg-yellow-50 transition-all">
        <div class="flex justify-between items-start mb-3">
            ...
            <button data-question-id="${q.id}" class="text-red-600 hover:text-red-800 ml-4">
...
document.addEventListener('DOMContentLoaded', () => {
    const askBtn = document.getElementById('toggle-ask-form-btn');
    askBtn?.addEventListener('click', toggleAskForm);

    const cancelBtn = document.getElementById('cancel-btn');
    cancelBtn?.addEventListener('click', toggleAskForm);

    const submitBtn = document.getElementById('submit-question-btn');
    submitBtn?.addEventListener('click', submitQuestion);

    // Delegated delete for dynamically added questions
